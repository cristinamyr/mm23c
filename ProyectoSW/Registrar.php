	...
	<div id='page-wrap'>
		...
		@session_start();
		    if (isset($_SESSION['email'])) {
	            echo '<script language="javascript">alert("¿Te quieres volver a registrar?¿o qué?");';
					...
					if(isset($_POST['email'])){
						...
						if (valoresCompletos($conn)){
							if ($_FILES["fichero"]["type"]){
							    ...
							    $sql = "INSERT INTO Usuarios (email, nombre, nick, password) VALUES ('$email', '$nombre', '$nick', '$hashPass')";
							}

							if (mysqli_query($conn, $sql)){
							    echo '<script language="javascript">alert("El usuario ha sido añadido correctamente.");';

							}else{
							    echo '<script language="javascript">alert("Como puede observar ha habido un problema al añadir al usuario en la base de datos.");';
							}
						}else{
							  echo '<script language="javascript">alert("La que has liado pollito, hay algún campo incorrecto. Vuelve a rellenar el formulario.");';
						}
