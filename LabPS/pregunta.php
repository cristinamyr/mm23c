...
<body>
	<div id='page-wrap'>
		...
		<section class="main" id="s1">
			<div>
				<form id="fpreguntas" method="post" name="preguntas" action="pregunta.php" method="post" enctype="multipart/form-data">
					E-mail(*):<input type="text"  size="50" id="email" name="email" value=<?php echo '"'.$_GET['email'].'"'?> disabled/><br>
					Enunciado(*): <input type="text" size="80"  id="enunciado" name="enunciado" pattern=".{10,}" required /><br>
					...
					Complejidad de la pregunta entre 1 y 5(*): <input type="text"  size="10" id="complejidad" name="complejidad" pattern="[1-5]" required /><br>
			...
			<?php 
				if(isset($_POST['email'])){
					...
					{
						if (is_valid_email($_POST['email']) && isset($_POST['enunciado']) && (strlen($_POST['enunciado'])>9) && isset($_POST['r_correcta']) && isset($_POST['r_inc_1']) && isset($_POST['r_inc_2']) && isset($_POST['r_inc_3']) && isset($_POST['complejidad']) && strlen($_POST['complejidad'] >0) && strlen($_POST['complejidad']<6) && isset($_POST['tema']) && (strlen($_POST['complejidad'])==1)){
							return true;
						}
						else{
							return false;
